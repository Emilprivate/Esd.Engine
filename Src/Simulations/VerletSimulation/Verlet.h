...
public:
    ...
    void RenderUI() override {
        ...
        if (ImGui::CollapsingHeader("Object Settings")) {
            // Set the width of the labels to make the elements align properly
            ImGui::SetNextItemWidth(150);

            // Create two columns to organize the sliders
            ImGui::Columns(2, nullptr, false);
            ImGui::SetColumnOffset(1, 200); // Adjust the offset between the columns

            // First column
            float object_spawn_delay = settings.GetSimulations().verlet.object_spawn_delay;
            if (ImGui::SliderFloat("Delay", &object_spawn_delay, 0.0f, 10.0f)) {
            ...
            if (ImGui::SliderFloat("Speed", &object_spawn_speed, 0.0f, 10.0f)) {
                settings.GetSimulations().verlet.object_spawn_speed = object_spawn_speed;
            }

            float object_radius = settings.GetSimulations().verlet.object_min_radius;
            if (ImGui::SliderFloat("Radius", &object_radius, 0.0f, 100.0f)) {
                settings.GetSimulations().verlet.object_min_radius = object_radius;
                settings.GetSimulations().verlet.object_max_radius = object_radius;
            }

            ImGui::NextColumn();

            uint32_t max_objects_count = settings.GetSimulations().verlet.max_objects_count;
            if (ImGui::SliderInt("Amount", (int*)&max_objects_count, 1, 1000)) {
            ...
            if (ImGui::SliderFloat("Angle", &max_angle, 0.0f, 2 * PI)) {
